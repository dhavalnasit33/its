...
function WhyChooseCard({ item, index }: CardProps) { 
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-3xl border-t-2 border-t-[#d68029] border border-slate-100 shadow-sm p-8 flex flex-col gap-6 hover:-translate-y-2 hover:shadow-2xl transition-all duration-[350ms] ease-out cursor-default w-full sm:w-[calc(50%-16px)] lg:w-[calc(33.333%-22px)]"
    >
      {/* Icon Container */}
      <div className={`w-14 h-14 rounded-2xl  bg-gray-50  flex items-center justify-center shrink-0 overflow-hidden transition-transform duration-300 group-hover:scale-105`}>
        <Image
          src={item.image}
          alt={item.title}
          width={48}
      ...
      <div className="flex flex-col gap-2">
        <h3 className="text-lg font-bold text-[#0d1b2a] leading-snug">
          {item.title}
        </h3>
        <p
          className="text-gray-500 text-sm sm:text-base leading-[1.8] line-clamp-4"
  ...
  return (
    ...
    <Row>
      ...
      <div className="relative z-10 w-[94%]  mx-auto flex flex-col gap-16">
        ...
        {/* ── Cards Grid (Using flex-wrap with center alignment) ── */}
        {items.length > 0 ? (
        ...
        <motion.div
          ...
          className="relative w-full bg-[#0a1628] rounded-[28px] px-8 py-10 md:px-14 md:py-12 flex flex-col md:flex-row items-center justify-between gap-8 overflow-hidden shadow-xl"
        >
