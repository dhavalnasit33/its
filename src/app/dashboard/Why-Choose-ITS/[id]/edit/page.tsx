'use client';

import { useEffect, useState } from "react";
import { useRouter, useParams } from "next/navigation";
import PageHeader from "@/components/shared/PageHeader";
import HomeChooseItsForm, { HomeChooseItsFormValues } from "@/components/dashboard/Why-Choose-ITS/HomeChooseItsForm";
import apiService from "@/lib/apiService";
import { useToast } from "@/hooks/use-toast";
import { Skeleton } from "@/components/ui/skeleton";
import type { HomeChooseIts } from "@/types";

export default function EditHomeChooseItsPage() {
  const router = useRouter();
  const { id } = useParams();
  const { toast } = useToast();
  const [item, setItem] = useState<HomeChooseIts | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const fetchItem = async () => {
      try {
        const res = await apiService<{ success: boolean; data: HomeChooseIts }>(`/choose_its_home/${id}`, {
          method: "GET",
        });

        if (res.success) {
          setItem(res.data);
        } else {
          toast({ title: "Error", description: "Failed to fetch entry data.", variant: "destructive" });
          router.push("/dashboard/Why-Choose-ITS");
        }
      } catch (error) {
        toast({ title: "Error", description: "Something went wrong.", variant: "destructive" });
        router.push("/dashboard/Why-Choose-ITS");
      } finally {
        setIsLoading(false);
      }
    };

    if (id) fetchItem();
  }, [id, router, toast]);

  const handleSubmit = async (data: HomeChooseItsFormValues) => {
    try {
      const res = await apiService<{ success: boolean; message: string }>(`/choose_its_home/${id}`, {
        method: "PUT",
        body: JSON.stringify(data),
        headers: { "Content-Type": "application/json" },
      });


      if (res.success) {
        toast({ title: "Success", description: "Entry updated successfully." });
        router.push("/dashboard/Why-Choose-ITS");
      } else {
        throw new Error(res.message || "Failed to update entry.");
      }
    } catch (error: any) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    }
  };

  return (
    <div className="space-y-6">
      <PageHeader
        title="Edit Why Choose ITS"
        description="Update the entry details"
        
      />
      <div className="rounded-xl border bg-card p-6 shadow-sm">
        {isLoading ? (
          <div className="space-y-4">
            <Skeleton className="h-10 w-full" />
            <Skeleton className="h-32 w-full" />
            <Skeleton className="h-10 w-32" />
          </div>
        ) : item ? (
          <HomeChooseItsForm
            initialData={item}
            onSubmit={handleSubmit}
            onCancel={() => router.push("/dashboard/Why-Choose-ITS")}
          />

        ) : (
          <p>Entry not found.</p>
        )}
      </div>
    </div>
  );
}
