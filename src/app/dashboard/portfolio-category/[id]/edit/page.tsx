...
export default function EditPortfolioCategoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        }>(`/portfolio-category/${id}`, { method: "GET" });
        ...
        if (res.success) {
          ...
          toast({
            title: "Error",
            description: "Failed to retrieve portfolio details",
            variant: "destructive",
          });
          router.push("/dashboard/portfolio-category");
        }
      } catch (error) {
        toast({
          title: "Error",
  ...
  const handleEdit = async (data: CategoryFormValues) => {
    try {
      const res = await apiService<{ success: boolean; message: string }>(`/portfolio-category/${id}`, {
        method: "PUT",
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Update Hire portfolio details"
      />
       {loading ? (
      ...
      <Card className="border border-slate-100 shadow-sm bg-white">
        <CardContent className="pt-6">
          <CategoryFrom 
            ...
            onCancel={() => router.push("/dashboard/portfolio-category")}
