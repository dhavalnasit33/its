...
async function checkCategoryUsage(categoryId) {
  const count = await Faq.countDocuments({ categories: categoryId });
  const reasons = [];
  if (count > 0) reasons.push(`${count} FAQ(s)`);
  return reasons;
}
...
router.post("/", protect, async (req, res) => {
  try {
    ...
    const existingCategory = await FaqCategory.findOne({ category });
    if (existingCategory) {
      return res.status(400).json({
        success: false,
    ...
    const newCategory = new FaqCategory({ category, image });
...
router.get("/", async (req, res) => {
  try {
    ...
    const [categories, total] = await Promise.all([
      FaqCategory.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
      FaqCategory.countDocuments(query),
    ]);
...
// @route   PUT /api/faq-category/:id
// @access  Private (Admin only)
router.put("/:id", protect, cleanupOldImages(FaqCategory, "FaqCategory"), async (req, res) => {
  try {
    const { category, image } = req.body;

    const categoryDoc = await FaqCategory.findById(req.params.id);
    if (!categoryDoc) {
      return res.status(404).json({
        success: false,
        message: "FAQ category not found",
...
// @route   DELETE /api/faq-category/:id
// @access  Private (Admin only)
router.delete("/:id", protect, cleanupImages(FaqCategory), async (req, res) => {
  try {
    const categoryDoc = await FaqCategory.findById(req.params.id);
    if (!categoryDoc) {
      return res.status(404).json({
...
// @route   POST /api/faq-category/bulk/delete
// @access  Private (Admin only)
router.post("/bulk/delete", protect, cleanupImages.cleanupBulkImages(FaqCategory), async (req, res) => {
  try {
    const { ids } = req.body;
