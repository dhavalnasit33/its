...
// @access  Private (Admin only)
router.put("/:id", protect, cleanupOldImages(HireCategory, "HireCategory"), async (req, res) => {
  try {
    const { category, image } = req.body;
...
// @access  Private (Admin only)
router.delete("/:id", protect, cleanupImages(HireCategory), async (req, res) => {
  try {
    const categoryDoc = await HireCategory.findById(req.params.id);
    if (!categoryDoc) {
      return res.status(404).json({
...
// @access  Private (Admin only)
router.post("/bulk/delete", protect, cleanupImages.cleanupBulkImages(HireCategory), async (req, res) => {
  try {
    const { ids } = req.body;
