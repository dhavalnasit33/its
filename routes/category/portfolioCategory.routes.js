...
async function checkCategoryUsage(categoryId) {
  const count = await CreativeWork.countDocuments({ category: categoryId });
  const reasons = [];
  if (count > 0) reasons.push(`${count} Portfolio Item(s)`);
  return reasons;
}
...
router.post("/", protect, async (req, res) => {
  try {
    ...
    const newCategory = new PortfolioCategory({ category, image });
...
router.get("/", async (req, res) => {
  try {
    ...
    const [categories, total] = await Promise.all([
      PortfolioCategory.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
      PortfolioCategory.countDocuments(query),
    ]);
...
// @route   GET /api/portfolio-category/:id
// @access  Private (Admin only)
...
router.put("/:id", protect, cleanupOldImages(PortfolioCategory, "PortfolioCategory"), async (req, res) => {
  try {
    ...
    const duplicate = await PortfolioCategory.findOne({
...
router.post("/bulk/delete", protect, cleanupImages.cleanupBulkImages(PortfolioCategory), async (req, res) => {
  try {
    ...
    for (const id of ids) {
      ...
      if (usage.length > 0) {
        const cat = await PortfolioCategory.findById(id);
