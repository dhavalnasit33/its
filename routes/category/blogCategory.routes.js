...
async function checkCategoryUsage(categoryId) {
  const [blogsCount, subcategoriesCount] = await Promise.all([
    Blog.countDocuments({ categories: categoryId }),
    BlogSubcategory.countDocuments({ category: categoryId }),
  ]);
...
router.post("/", protect, async (req, res) => {
  try {
    ...
    const newCategory = new BlogCategory({ category, image });
