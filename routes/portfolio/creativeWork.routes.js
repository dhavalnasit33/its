...
/**
 * @swagger
 * /api/creative-work-random:
 *   get:
 *     summary: Get 10 random portfolio creative works
 *     tags: [CreativeWorkRandom]
 *     responses:
 *       200:
 *         description: List of 10 random creative works
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *         description: Server error
 */
router.get("/creative-work-random", async (req, res) => {
  try {
    const rawCreativeWorks = await CreativeWork.aggregate([
      { $sample: { size: 10 } },
    ]);

    const creativeWorks = await populateMixed(rawCreativeWorks, {
      category: { type: "category", model: PortfolioCategory },
    });
    ...
    console.error("❌ Error fetching random creative works:", error);
    res.status(500).json({
      success: false,
      message: "Server Error",
    });
  ...
  try {
    ...
    if (category) {
      if (mongoose.Types.ObjectId.isValid(category)) {
        ...
        const catDoc = await PortfolioCategory.findOne({
          category: category.trim(),
        });
        if (catDoc) {
          query.$or = [{ category: category }, { category: catDoc._id }];
        } else {
          query.category = category;
        }
      }
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const data = await populateMixed(rawData, {
      category: { type: "category", model: PortfolioCategory },
    });
    res.status(200).json({
      success: true,
...
router.put(
  "/:id",
  protect,
  cleanupOldImages(CreativeWork, "CreativeWork"),
  async (req, res) => {
    try {
      const { category, title, url, image } = req.body;
      if (!category || !title || !image) {
...
router.post(
  "/bulk-delete",
  protect,
  cleanupImages.cleanupBulkImages(CreativeWork),
  async (req, res) => {
    try {
      const { ids } = req.body;
